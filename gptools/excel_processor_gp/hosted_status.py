...
def _credentials():
    ...
    username = os.getenv("AMSA_ADMIN_USER")
    password = os.getenv("AMSA_ADMIN_PASSWORD")
    if username and password:
        return portal, username, password
    path = Path(os.getenv("AMSA_CREDENTIALS_FILE", str(LOCAL_CREDENTIALS)))
    if not path.exists():
        raise RuntimeError("No se configuro AMSA_CREDENTIALS_FILE ni AMSA_ADMIN_USER/AMSA_ADMIN_PASSWORD")
...
def _token(portal, username, password):
    result = _post(f"{portal}/sharing/rest/generateToken", {
...
def update_status(key, process, filename, records, session_user="", full_name="", email="", origin="GP Tool / REST", message=""):
    try:
        ...
        if not edit_result.get("success"):
            raise RuntimeError(str(edit_result.get("error") or "applyEdits no confirmado"))
        return {"actualizado": True, "operacion": operation, "objectid": edit_result.get("objectId")}
